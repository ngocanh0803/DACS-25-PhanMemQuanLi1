...
if ($action === 'approve') {
    ...
    $sqlUpdate = "UPDATE Departure_Requests SET status = 'approved', processed_date = NOW() WHERE departure_id = ?";
    ...
    if ($user_id) {
        ...
        $notif_message = "Đơn xin rời phòng do hết hạn hợp đồng của bạn đã được duyệt. Vui lòng hoàn tất thủ tục bàn giao.";
    ...
    echo json_encode(['success' => true, 'message' => 'Đơn rời phòng (hết hạn) đã được duyệt.']);
} 
elseif ($action === 'reject') {
    $reason = isset($data['reason']) ? trim($data['reason']) : '';
    // Từ chối => update departure_requests
