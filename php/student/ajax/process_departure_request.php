...
if (empty($reason)) {
    $response['errors'][] = 'Lý do xin rời phòng là bắt buộc.';
...
if (isset($_FILES['documents']) && $_FILES['documents']['error'][0] != UPLOAD_ERR_NO_FILE) {
    ...
    // Đường dẫn đã sửa (đơn giản và chính xác hơn):
    $absoluteUploadDir = dirname(__FILE__) . '/../uploads/departure_documents/';
...
$sqlInsertRequest = "INSERT INTO Departure_Requests (student_id, contract_id, reason, documents, status) VALUES (?, ?, ?, ?, 'pending')";
$stmtRequest = $conn->prepare($sqlInsertRequest);
if (!$stmtRequest) {
    $response['errors'][] = 'Lỗi chuẩn bị truy vấn request: ' . $conn->error;
    echo json_encode($response);
    exit();
}
$stmtRequest->bind_param("iiss", $studentId, $contractId, $reason, $documentsJson);

if ($stmtRequest->execute()) {
    $response['success'] = true;
    $response['message'] = 'Đơn xin rời phòng đã được gửi thành công.';
} else {
    $response['errors'][] = 'Lỗi khi gửi đơn: ' . $stmtRequest->error;
}
