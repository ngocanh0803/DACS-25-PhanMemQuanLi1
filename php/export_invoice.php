...
if (isset($_GET['payment_id'])) {
    ...
    // Tạo PDF
    $pdf = new TCPDF();
    $pdf->SetMargins(15, 15, 15); // Điều chỉnh lề cho phù hợp
    $pdf->AddPage();

    // Đặt font cho tiếng Việt (Sử dụng DejaVu Sans - đã được thử nghiệm)
    $pdf->SetFont('dejavusans', '', 12);

    // Đường dẫn logo tuyệt đối
    $logoPath = '/Applications/XAMPP/xamppfiles/htdocs/KTX_PTTKPM_ADMIN/assest/img/logohoadon.png';

    // Nội dung HTML (Cải thiện giao diện với màu sắc và kích thước chữ hợp lý)
    $html = '
    ...
    // Chèn Logo vào cuối trang và căn giữa
    if (file_exists($logoPath)) {
        // Chiều cao và chiều rộng của logo (mm)
        $logoWidth = 50; // Điều chỉnh theo nhu cầu
        $logoHeight = 30; // Điều chỉnh theo nhu cầu

        // Lấy chiều rộng và chiều cao của trang
        $pageWidth = $pdf->getPageWidth();
        $pageHeight = $pdf->getPageHeight();

        // Tính toán vị trí X và Y để căn giữa logo ở cuối trang
        $xPosition = ($pageWidth - $logoWidth) / 2;
        $yPosition = $pageHeight - $pdf->getMargins()['bottom'] - $logoHeight - 10; // 10mm thêm lề dưới

        // Thêm Logo vào cuối trang
        $pdf->Image($logoPath, $xPosition, $yPosition, $logoWidth, $logoHeight, '', '', '', false, 300, '', false, false, 0, false, false, false);
    } else {
        // Xử lý khi không tìm thấy logo, ví dụ: ghi log
